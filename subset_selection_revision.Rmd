...
write_dir <- 'subset_summary_metrics_revise/'
fig_dir <- 'figures_revise/'
...
region_summary_main %>%
  filter(experiment != 'ssp119',
    ...
    mutate(hist_val_end = end_anomaly-end_val,
           hist_val_mid = mid_anomaly-mid_century_val)->
  region_summary 
...
# see how eigenvectors change

```{r}
orig_eval <- read.csv('orig_eval.csv')
orig_evec <- read.csv('orig_evec.csv')


max(abs(full_eigenval - orig_eval$eigenvalues[1:6]))

max(abs(orig_evec$PC1 - as.data.frame(full_eigenvec)$PC1))
max(abs(orig_evec$PC2 - as.data.frame(full_eigenvec)$PC2))
max(abs(orig_evec$PC3 - as.data.frame(full_eigenvec)$PC3))
max(abs(orig_evec$PC15 - as.data.frame(full_eigenvec)$PC15))
```
