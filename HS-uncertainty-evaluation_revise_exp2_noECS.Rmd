---
title: "HS uncertainty evalution Experiment 2 - independent models, no ECS constraint"
author: "acs"
output: 
  html_document:
    toc: true
...
Choose ESMs with ECS constraint

esms_min_exp1 = ['ACCESS-CM2',   'ACCESS-ESM1-5', 'BCC-CSM2-MR', 'MIROC6', 'MRI-ESM2-0']

esms_min_exp2 = ['IPSL-CM6A-LR', 'ACCESS-ESM1-5',  'MRI-ESM2-0', 'MPI-ESM1-2-LR', 'MIROC6']


Choose ESMs without ECS constraint

esms_min_exp1_noECS = ['BCC-CSM2-MR', 'CESM2', 'CMCC-ESM2', 'MRI-ESM2-0', 'UKESM1-0-LL']

esms_min_exp2_noECS = ['CESM2', 'IPSL-CM6A-LR', 'MRI-ESM2-0', 'MPI-ESM1-2-LR', 'UKESM1-0-LL']
...
pr_coef <- pr_coef[(grepl('noECS', pr_coef))]
...
tas_coef <- tas_coef[(grepl('noECS', tas_coef))]
...
p_tas <- ggplot()+
    ...
    ggtitle(paste('Temperature Experiment 2, rms >', rms_cutoff, 'no ECS constraint')) +
    theme_bw() +
...
ggsave(paste0(fig_dir, 'temperature_exp2_', rms_cutoff,'_HS_regions_noECS.png'), p_tas, width = 8, height =6, units = 'in')
...
p_pr <- ggplot()+
    ...
    facet_wrap(~rgn, ncol=6) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    ggtitle(paste('Precipitation Experiment 2, rms >', rms_cutoff, 'no ECS constraint'))+
    theme_bw() +
     theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
           axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
           axis.title = element_text(size = axis_size), 
...
ggsave(paste0(fig_dir, 'precipitation_exp2_', rms_cutoff,'_HS_regions_noECS.png'), p_pr, width = 8, height =6, units = 'in')
...
p_map <- ggplot() +
 ...
 #   scale_fill_brewer() +
  ...
  #ggtitle('coef') + 
    ...
    ggtitle(paste('Experiment 2, rms cutoff', rms_cutoff, 'no ECS constraint'))


p_map

ggsave(paste0(fig_dir, 'HS_success_map_', rms_cutoff, '_noECS.png'), p_map, width = 14, height = 6, units = 'in')
