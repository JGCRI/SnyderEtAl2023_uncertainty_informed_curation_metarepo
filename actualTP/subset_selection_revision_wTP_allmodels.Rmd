...
write_dir <- 'subset_summary_metrics_revise_tp_allmodels/'
...
subset_ineligible <- c('NorESM2-MM', 'CMCC-CM2-SR5' )
...
var_explained_df %>%
  ...
  geom_point(size=4)+
  geom_line()+
  labs(title="Scree plot: PCA on scaled data, Experiment 1")
...
p_map <- ggplot() +
  geom_sf(data = shp2 %>% na.omit(), mapping = aes(fill = value, color=value)  ) +
    ...
    ggtitle('Experiment 1 eigenvectors')
...
# remove any other ESMs from candidacy to be selected:
table_of_subsets %>%
    filter(!(subset %in% subset_ineligible)) %>%
    group_by(subset_id) %>%
    mutate(nmodels = n()) %>%
    ungroup %>%
    filter(nmodels == 5) %>%
...
ggsave(paste0(fig_dir, 'full_data_coeff_grid_plot.png'), width = 9.5, height =6.5, units = 'in')
ggsave(paste0(fig_dir, 'full_data_coeff_grid_plot.jpg'), width = 9.5, height =6.5, units = 'in')
...
ggsave(paste0(fig_dir, 'full_data_coeff_grid_plot_subset.png'), width = 9.5, height =6.5, units = 'in')
ggsave(paste0(fig_dir, 'full_data_coeff_grid_plot_subset.jpg'), width = 9.5, height =6.5, units = 'in')
