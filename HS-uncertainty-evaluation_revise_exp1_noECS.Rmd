---
title: "HS uncertainty evalution Experiment 1 -  all models, no ECS constraint"
author: "acs"
output: 
  html_document:
    toc: true
...
# filter to exp1
pr_list <- pr_list[grepl('exp1', pr_list)]
tas_list <- tas_list[grepl('exp1', tas_list)]
...
p_tas <- ggplot()+
    ...
    ggtitle(paste('Temperature Experiment 1, rms >', rms_cutoff, 'no ECS constraint')) +
    theme_bw() +
...
ggsave(paste0(fig_dir, 'temperature_exp1_', rms_cutoff,'_HS_regions_noECS.png'), p_tas, width = 8, height =6, units = 'in')
...
p_pr <- ggplot()+
    ...
    facet_wrap(~rgn, ncol=6) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    ggtitle(paste('Precipitation Experiment 1, rms >', rms_cutoff, 'no ECS constraint'))+
    theme_bw() +
     theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
           axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
           axis.title = element_text(size = axis_size), 
...
ggsave(paste0(fig_dir, 'precipitation_exp1_', rms_cutoff,'_HS_regions_noECS.png'), p_pr, width = 8, height =6, units = 'in')
...
p_map <- ggplot() +
  ...
  #ggtitle('coef') + 
    ...
    ggtitle(paste('Experiment 1, rms cutoff of ', rms_cutoff, 'no ECS constraint'))
